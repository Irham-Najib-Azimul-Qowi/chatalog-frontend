...
function AdminDashboardPage() {
  ...
  const [selectedImage, setSelectedImage] = useState(null); // Untuk preview gambar landing page
  const [imageFile, setImageFile] = useState(null); // Untuk file gambar landing page yang akan diupload
  const navigate = useNavigate();

  // State untuk modal produk
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingProduct, setEditingProduct] = useState(null); // null untuk 'tambah', objek produk untuk 'edit'
  ...
  // Menangani pemilihan file gambar landing page
  ...
  const handleSaveSettings = async () => {
    ...
    // Siapkan semua data teks untuk diupdate
    const textSettings = {
    ...
    // Jika ada gambar baru, siapkan untuk diupload
    if (imageFile) {
  ...
  // Fungsi untuk menyimpan (create & update) produk
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="mt-1 p-4 border-2 border-dashed border-gray-300 rounded-md text-center">
                  <img 
                    src={selectedImage || (settings.landing_page_image ? `http://127.0.0.1:8000/storage/${settings.landing_page_image}` : 'https://via.placeholder.com/400x200?text=Pilih+Gambar')} 
                    alt="Preview" 
